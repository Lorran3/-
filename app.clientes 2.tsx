...
function ClientesPage() {
  ...
  const updateNivel = (cliente: Profile, novoNivel: string) => {
    if (cliente.nivel === novoNivel) return;
    setMotivoNivel("");
    setNivelTarget({ cliente, novoNivel });
  };
  ...
  const confirmarAlteracaoNivel = async () => {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ nivel: novoNivel as "iniciante" | "intermediario" | "avancado" })
      .eq("id", cliente.id);
    ...
    if (u.user) {
      await supabase.from("nivel_alteracoes").insert({
        ...
        nivel_anterior: cliente.nivel ?? null,
        nivel_novo: novoNivel,
        motivo: motivoNivel.trim(),
      } as never);
    }
    setClientes((cs) => cs.map((c) => (c.id === cliente.id ? { ...c, nivel: novoNivel } : c)));
  ...
  const filtrados = clientes.filter((c) => {
    ...
    const nivelOk =
      filtroNivel === "todos" ||
      c.nivel === filtroNivel ||
      (c.categoria_arena ?? "").toLowerCase() === filtroNivel.toLowerCase();
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="px-4 -mt-14 relative z-10 space-y-3">
        ...
        {categoriasArena.length > 0 && (() => {
          const palette = [
            { bg: "bg-amber-500/15", text: "text-amber-700", Icon: Sparkles },
            { bg: "bg-sky-500/15", text: "text-sky-700", Icon: Zap },
            { bg: "bg-violet-500/15", text: "text-violet-700", Icon: Trophy },
            { bg: "bg-emerald-500/15", text: "text-emerald-700", Icon: UserCheck },
            { bg: "bg-rose-500/15", text: "text-rose-700", Icon: Users },
            { bg: "bg-indigo-500/15", text: "text-indigo-700", Icon: Sparkles },
          ];
          const cols = categoriasArena.length === 1 ? "grid-cols-1"
            : categoriasArena.length === 2 ? "grid-cols-2"
            : categoriasArena.length === 4 ? "grid-cols-2"
            : "grid-cols-3";
          return (
            <div className={`grid ${cols} gap-3`}>
              {categoriasArena.map((cat, i) => {
                const style = palette[i % palette.length];
                const Icon = style.Icon;
                const count = clientes.filter(
                  (c) => (c.categoria_arena ?? "").toLowerCase() === cat.toLowerCase()
                ).length;
                return (
                  <button
                    key={cat}
                    type="button"
                    onClick={() => setFiltroNivel(filtroNivel === cat ? "todos" : cat)}
                    className={`text-left rounded-2xl bg-card border shadow-card p-3 active:scale-[0.98] transition ${filtroNivel === cat ? "border-primary ring-2 ring-primary/30" : "border-border"}`}
                  >
                    <div className={`h-9 w-9 rounded-xl flex items-center justify-center mb-2 ${style.bg} ${style.text}`}>
        ...
        {filtrados.map((c) => (
          <Card key={c.id} className="p-3 rounded-2xl space-y-2 border-border/60 hover:shadow-card transition">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {canEditNivel ? (
              <Select value={c.nivel ?? ""} onValueChange={(v) => updateNivel(c, v)}>
      ...
      <Dialog open={!!nivelTarget} onOpenChange={(o) => !o && setNivelTarget(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Alterar nível</DialogTitle></DialogHeader>
          {nivelTarget && (
            <div className="space-y-3">
              <p className="text-sm">
                Mudando <b>{nivelTarget.cliente.nome}</b> de{" "}
                <span className="uppercase text-xs">{nivelTarget.cliente.nivel ?? "—"}</span> para{" "}
